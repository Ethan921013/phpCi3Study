...
//			$this->options[CURLOPT_CONNECTTIMEOUT] = 10;
	...
	public function curlExec($param){

        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $param['CURLOPT_URL']);
        curl_setopt($curl, CURLOPT_CONNECTTIMEOUT, 3);
        curl_setopt($curl, CURLOPT_TIMEOUT, 5);
        curl_setopt($curl, CURLOPT_HEADER, FALSE);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);

        if(isset($param['CURLOPT_HTTPHEADER'])){
            curl_setopt($curl, CURLOPT_HTTPHEADER, $param['CURLOPT_HTTPHEADER']);
        }
        if(isset($param['CURLOPT_CONNECTTIMEOUT'])){
            curl_setopt($curl, CURLOPT_CONNECTTIMEOUT, $param['CURLOPT_CONNECTTIMEOUT']);
        }
        if(isset($param['CURLOPT_TIMEOUT'])){
            curl_setopt($curl, CURLOPT_TIMEOUT, $param['CURLOPT_TIMEOUT']);
        }
        if(isset($param['CURLOPT_POSTFIELDS'])){
            curl_setopt($curl, CURLOPT_POST, true);
            curl_setopt($curl, CURLOPT_POSTFIELDS, $param['CURLOPT_POSTFIELDS']);
        }
